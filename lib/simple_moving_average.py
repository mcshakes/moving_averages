...
class SimpleMovingAverage:
    ...
    def plot_avgs_crossover(self, data):
        plt.figure(figsize=(20, 10), dpi=80)
        plt.plot(data.index, data['Close'])
        plt.plot(data.index, data['2_SMA'], '-^',
                 markevery=self.trade_buy, ms=15, color='green')
        plt.plot(data.index, data['5_SMA'], '-v',
                 markevery=self.trade_sell, ms=15, color='red')
